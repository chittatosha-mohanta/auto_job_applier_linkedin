'''
Author:     Chittatosha Mohanta
LinkedIn:   https://www.linkedin.com/in/chittatosha-mohanta-32a182288/

Copyright (C) 2026 Chittatosha Mohanta

License:    GNU Affero General Public License
            https://www.gnu.org/licenses/agpl-3.0.en.html
            
GitHub:     https://github.com/chittatosha-mohanta/auto_job_applier_linkedin
...
try:
    ...
    if os.path.exists(db_path):
        ...
        cur = conn.cursor()
        cur.execute("SELECT id, cv_path FROM users WHERE is_active_bot_user = 1 LIMIT 1")
        row = cur.fetchone()
        if row:
            ACTIVE_BOT_USER_ID = row['id']
            if row['cv_path']:
                default_resume_path = row['cv_path']
                print(f"Logged in as Active DB User #{ACTIVE_BOT_USER_ID}. Using CV: {default_resume_path}")
        conn.close()
except Exception as e:
    print("Error fetching active user from DB:", e)
# ----------------------------------------------------
...
def submitted_jobs(job_id: str, title: str, company: str, work_location: str, work_style: str, description: str, experience_required: int | Literal['Unknown', 'Error in extraction'], 
    ...
    try:
        ...
        # Database Insertion for Web Portal tracking
        if ACTIVE_BOT_USER_ID is not None:
            try:
                db_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'instance', 'site.db')
                conn = sqlite3.connect(db_path)
                cur = conn.cursor()
                application_time = str(date_applied) if date_applied != 'Pending' else datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                cur.execute('''
                    INSERT INTO job_applications (user_id, job_id, title, company, hr_name, hr_link, job_link, external_link, date_applied)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                ''', (ACTIVE_BOT_USER_ID, truncate_for_csv(job_id), truncate_for_csv(title), truncate_for_csv(company), truncate_for_csv(hr_name), truncate_for_csv(hr_link), truncate_for_csv(job_link), truncate_for_csv(application_link), application_time))
                conn.commit()
                conn.close()
            except Exception as e:
        ...
        try:
            while current_count < switch_number:
                ...
                for job in job_listings:
                    ...
                    if (easy_applied_count + external_jobs_count) >= APPLICATION_LIMIT:
                        print_lg(f"\n###############  Application limit of {APPLICATION_LIMIT} reached!  ###############\n")
                        return
...
def main() -> None:
    try:
        ...
        msg = f"\n{quote}\n\n\nBest regards,\nChittatosha Mohanta\nhttps://www.linkedin.com/in/chittatosha-mohanta-32a182288/\n\n"
